...
i=0
#filtered_file_t=[filtered_file[78]]
for path in filtered_file:
    ...
    for line in Lines:
        if "forrtl: severe" in line:
            idx_ana= path.index('/ANA/') + len('/ANA/') # to find in what position /ANA/ finishes
            date1=(path[idx_ana:idx_ana + 8])
            fortran_bug_stk.append(date1)
    ...
    if  HeadDate[0] and HeadDate[1]:
        ...
        idx_ana= path.index('/ANA/') + len('/ANA/') # to find in what position /ANA/ finishes
        date2=(path[idx_ana:idx_ana + 8])
        date3 = date2+'_0002'
        commet = 'CASCADIA_AMB'
        for sta in sta_amb:    
            nfila = nf0+' '+str(t0_min)+' '+str(t1_max)+' '+str(sta)+' '+' '+'0'+'   '+date3+' '+commet
        ...
        for k in df_amb_f_2.index:
            ...
            nfila_2 = nf0+' '+str(t0)+' '+str(t1)+' '+str(sta)+' '+' '+str(sat)+'   '+date3+' '+commet
...
for path in filtered_file:
    ...
    for line in Lines:
        if "**** error : number of records (amb_file) : 40001" in line:
            idx_ana= path.index('/ANA/') + len('/ANA/') # to find in what position /ANA/ finishes
            residual_high_stk.append(path[idx_ana:idx_ana + 8])
date_residual_high_stk = [fila.replace("_", "") for fila in residual_high_stk]

#residual_high_stk=['2015_012']
residual_excuded=['2016_178','2016_179','2016_205','2016_206', '2016_213', '2016_216']


residual_high_stk = [elemento for elemento in residual_high_stk if elemento not in residual_excuded]
...
for yyyy_doy in residual_high_stk:
    ...
    if yyyy==year:
        ...
        nf0 =' DEL'
        idx_ana= path.index('/ANA/') + len('/ANA/') # to find in what position /ANA/ finishes
        date=(dire_res_sat[idx_ana:idx_ana + 8])
        date = date+'_0003'
...
input_file_path = "/home/garcia/wrk_garcia/CONTROL/CNT_CASCADIA_NET_MIN_240209_G_repro3/CLN_GLOBAL_CASCADIA_NET"
...
config_opera = '/home/garcia/wrk_garcia/CONFIG_NET_CASCADIA_240209_MIN.xml'
...
residual_excuded=['2016178','2016179','2016205','2016206', '2016213', '2016216']

date_residual_high_stk= [elemento for elemento in date_residual_high_stk if elemento not in residual_excuded]



res_line_stk=[]
date_residual_high_stk_1=[]
for day in date_residual_high_stk:
    ...
    #kommand_gnss_amb =  "epos_start -cfg " + config_opera + " -beg_day " + day + ' -rod save ; /bin/rm -rf /dsk1/tmp/garcia/\*'
        ...
        date_residual_high_stk_1.append(day)

date_residual_high_stk_1 = [fila.replace("_", "")[:4] + "_" + fila.replace("_", "")[4:] for fila in date_residual_high_stk_1]



#res_line_stk=[res_line_stk[0]]
...
dir_path_ana='/home/garcia/wrk_garcia/CASCADIA_NET/ANA/'
dir_path_cln='/home/garcia/wrk_garcia/CASCADIA_NET/CLN/'
...
for opt in options:
    try:
        if opt =='fortran':
            ...
            ver='01_RES'
            for folder in dir_path:
                for file_name in date_residual_high_stk_1:
                    file_path = os.path.join(folder,file_name)
